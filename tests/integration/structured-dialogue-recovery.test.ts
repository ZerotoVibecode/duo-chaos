...
class CorrectedClaudeDialogueRunner implements ProcessRunnerPort {
  readonly calls: Array<{ agent: 'claude' | 'codex'; round: number; stage: string }> = []
      ...
      options.onLine('stdout', JSON.stringify([
        {
          ...
          message: {
            content: [{
              ...
              input: { __unparsedToolInput: '{"statement":' }
            }]
          }
        },
        {
          ...
          message: {
            content: [{
              type: 'tool_result',
              is_error: true,
              content: 'InputValidationError: Correct the malformed JSON and retry StructuredOutput.'
            }]
          }
        },
        {
          type: 'assistant',
          message: {
            content: [{ type: 'tool_use', name: 'StructuredOutput', input: move }]
          }
        },
        {
          type: 'result',
...
describe('structured dialogue recovery', () => {
  it('accepts a valid second Claude schema submission in the same bounded provider call', async () => {
    const root = await mkdtemp(join(tmpdir(), 'duo-structured-second-submit-'))
    const runner = new CorrectedClaudeDialogueRunner()
    ...
    expect(snapshot?.pause?.reason).not.toBe('cli-incompatible')
