...
- Provider quota advisories appear as live pressure events and trigger an automatic checkpoint without interrupting an allowed call sequence. Only an explicit provider quota rejection pauses the entire duel before the opponent moves. The workspace, balanced turn cursor, usage, and evidence remain resumable; Duo neither retries blindly nor permits a one-agent takeover.
- Completed-call usage guards use only exact terminal fields reported by the local CLI. The conservative default boundary is more than 250,000 effective input tokens—calculated as uncached input plus 10% of cached input—or 24,000 output or reasoning tokens in one call. Crossing it finishes the active call, checkpoints the workspace, records an advisory, and continues from a fresh compact baton while the provider still accepts calls. Raw processed and cached totals remain visible in telemetry. This is an observability boundary, not a quality claim, token estimate, or artificial stop condition.
...
`npm run benchmark:live` is also a zero-call dry run by default. The provider-backed path accepts no prompt or loadout overrides and requires both `--live` and `--i-understand-this-uses-local-cli-quota` after a production build. It runs one fixed Terra Low + Sonnet Low condition in isolated Electron user data and a temporary workspace, forces raw-log saving off, never auto-resumes, and records every terminal outcome instead of hiding weak samples. The generated workspace is retained under ignored `benchmark-results/` for blinded human review; this archive is separate from Playwright's disposable `test-results/` output. Its restricted judge uses supervisor evidence only; blinded human review is still required for product-quality claims. See [docs/BENCHMARKING.md](docs/BENCHMARKING.md).

`npm run benchmark:matrix` is a separate zero-call dry run by default. Its doubly gated live path executes exactly one explicit arm and trial from an immutable, canonical-prompt-hashed Decision Deck suite, never retries or resumes, and preserves every result below an opaque ignored `benchmark-results/short-matrix/` run ID. The default `low-diagnostic` suite and `premium-medium` suite keep their historical 15-minute contracts and slots unchanged. `--suite premium-medium-open` selects a separate Sol Medium plus Opus Medium continuation with no independent harness active-work wall; `codex-effort-open-v1` isolates the Codex effort ladder. The Sol/Fable suites remain separate reliability records: v1 preserves the Windows Claude schema-transport failure, v2 preserves the provider-schema portability failure, and v3 preserves the first provider-compatible artifact whose package-free `node:test` proof was not recognized by the older supervisor. `sol-fable-2x2-open-v4` is the fresh post-supervisor-proof suite and keeps the same canonical prompt, loadouts, trials, judge contract, and open runtime. `sol-fable-low-match-open-v1` is the cost-focused matched comparison: Sol Low solo, Fable Low solo, then Sol Low + Fable Low Duo, each over trials 1 and 2 with the same task and judge. Open suites wait for a supervisor terminal result while retaining the application's eight-hour turn and 24-hour run safety leases. Each suite/arm/trial slot can be attempted only once. Duo preflight runs before reservation, and every successful reservation names a receipt that is written even for an unexpected harness failure. Solo arms use shell-free local CLI argument arrays, allowlisted environments, and Core toolsets; Duo arms use the Electron supervisor. Run `npm run benchmark:matrix:judge -- <preserved-workspace>` afterward for identical repo-owned hidden browser checks and bounded screenshots. The judge never executes agent-authored tests, but it validates a substantive `logic.test.mjs`, imports the exported deterministic logic contract, disables service workers, and blocks both external requests and WebSockets. Deterministic checks are evidence, not a substitute for blinded artifact review.
