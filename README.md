...
- Completed-call usage guards use only exact terminal fields reported by the local CLI. The conservative default boundary is more than 400,000 processed-input tokens, 24,000 output tokens, or 24,000 reasoning tokens in one call. Crossing it finishes the active call, checkpoints the workspace, and pauses only before another premium call. It is a safety boundary, not a quality claim or a token estimate; explicit Resume grants one fresh compact call from the durable baton.
