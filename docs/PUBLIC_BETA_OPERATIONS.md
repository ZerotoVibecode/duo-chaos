...
- Both providers share one soft work guard. Productive tools always finish; only a genuine idle loop is cancelled. Exact completed-call usage above 250,000 effective input tokens—uncached input plus 10% of cached input—or 24,000 output or reasoning tokens is checkpointed and pauses before another call. Raw processed and cached totals remain visible in telemetry. These are safety boundaries over provider-reported fields, not estimates or quality claims. Resume grants one fresh compact call.
