    ...
    <div className="sheet-scrim" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) onClose() }}>
      <aside ref={dialogRef} className="settings-sheet" role="dialog" aria-modal="true" aria-labelledby="settings-title" tabIndex={-1} onKeyDown={handleDialogKeyDown}>
        ...
        <div className="settings-content">
          ...
          <section>
            <h3>Toolbelts & quality routing</h3>
            <p className="settings-help">Smart keeps the prompt lean: source and review get Duo's compact quality skill plus connected MCP/app/plugin tools while skill catalogs stay off. Broad loads every user and plugin skill and can use substantially more context and quota.</p>
            <div className="settings-numbers">
              <label className="settings-field"><span>Codex toolbelt</span><select value={draft.codexCustomizationProfile} onChange={(event) => update('codexCustomizationProfile', event.target.value as AppSettings['codexCustomizationProfile'])}><option value="smart">Smart · Duo + connected tools</option><option value="core">Core · no user capabilities</option><option value="full-local">Broad · all user skills</option></select></label>
              <label className="settings-field"><span>Claude toolbelt</span><select value={draft.claudeCustomizationProfile} onChange={(event) => update('claudeCustomizationProfile', event.target.value as AppSettings['claudeCustomizationProfile'])}><option value="smart">Smart · Duo + connected tools</option><option value="core">Core · no user capabilities</option><option value="full-local">Broad · all user skills</option></select></label>
              <label className="settings-field"><span>Quality routing</span><select value={draft.qualityRoutingProfile} onChange={(event) => update('qualityRoutingProfile', event.target.value as AppSettings['qualityRoutingProfile'])}><option value="balanced">Balanced · stage-routed quality</option><option value="force-selected">Always use selected effort</option></select></label>
              <label className="settings-field"><span>Agent work inference lease</span><input type="number" min="3" max="20" value={draft.workInferenceLimit} onChange={(event) => update('workInferenceLimit', Number(event.target.value))} /></label>
            </div>
            <label className="toggle-field"><input type="checkbox" checked={draft.trustedLocalCapabilitiesConfirmed} onChange={(event) => update('trustedLocalCapabilitiesConfirmed', event.target.checked)} /><span><strong>Trust my local CLI capabilities</strong><small>Required for Smart/Broad in Real Mode. Smart suppresses the global user-skill catalog; Broad does not. Duo never stores capability credentials.</small></span></label>
            <p className="settings-help">Balanced routing treats each selected model as a ceiling, uses High implementation and Extra High review when the ceiling supports them, and keeps dialogue lean. The provider-neutral inference lease stops idle loops without discarding recorded tool evidence. Provider or runtime boundaries pause safely between calls for a compact resume.</p>
          </section>
          ...
          <section>
            <h3>Run limits</h3>
            <p className="settings-help">The normal path is seven calls: four lean debates, two deep contributions, and one reciprocal review stage that records both agents' current-revision review receipts. Extra repairs run only after recorded evidence finds a defect.</p>
            <div className="settings-numbers">
          ...
          <div className="settings-warning">Real Mode launches authenticated local CLIs and may consume paid usage. Source contributions use fresh compact sessions. Usage warnings create automatic checkpoints; only a rejected provider limit suspends the battle. Duo Chaos sends no product analytics or Duo-owned telemetry. Provider CLIs keep their own local account and history behavior.</div>
        </div>
